SELECT
    ...
    (COALESCE(SUM(CASE WHEN has_availability = 't' THEN 1 ELSE 0 END), 0) - COALESCE(LAG(SUM(CASE WHEN has_availability = 't' THEN 1 ELSE 0 END), 1) OVER (PARTITION BY listing_neighbourhood ORDER BY TO_CHAR(scraped_date, 'YYYY-MM')), 0)) * 100
    / COALESCE(LAG(SUM(CASE WHEN has_availability = 't' THEN 1 ELSE 0 END), 1) OVER (PARTITION BY listing_neighbourhood ORDER BY TO_CHAR(scraped_date, 'YYYY-MM')), 0) AS percentage_change_active_listings,
    (COALESCE(SUM(CASE WHEN has_availability = 'f' THEN 1 ELSE 0 END), 0) - COALESCE(LAG(SUM(CASE WHEN has_availability = 'f' THEN 1 ELSE 0 END), 1) OVER (PARTITION BY listing_neighbourhood ORDER BY TO_CHAR(scraped_date, 'YYYY-MM')), 0)) * 100
    / COALESCE(LAG(SUM(CASE WHEN has_availability = 'f' THEN 1 ELSE 0 END), 1) OVER (PARTITION BY listing_neighbourhood ORDER BY TO_CHAR(scraped_date, 'YYYY-MM')), 0) AS percentage_change_inactive_listings
